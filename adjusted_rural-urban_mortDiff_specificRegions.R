# This code calculates and plots the adjusted rural-urban mortality differences, where:
# the Greater London region is omitted
# only West Germany is considered

# Summarize years
# Reduce to variables necessary and bind GER and ENW
london <- read.csv("londonAreas20200207.tsv", header = TRUE,
                   sep ="", stringsAsFactors = FALSE)[-34,]

ENW <- 
  datENW %>% 
  filter(!Geo %in% london$geogcode) %>% 
  group_by(Age, Sex, Geo, PDcat) %>% 
  summarize(Nx = sum(Pop), Dx = sum(Death)) %>% 
  mutate(Country = "ENW")

## 11 = Berlin, 12-16 = new states
GER <- 
  datGER %>%
  mutate(State = case_when(nchar(Geo) == 4 ~ substr(Geo, 1, 1), 
                           nchar(Geo) == 5 ~ substr(Geo, 1, 2))) %>% 
  filter(as.numeric(State) < 12) %>% 
  group_by(Age, Sex, Geo, PDcat) %>% 
  summarize(Nx = sum(Pop), Dx = sum(Death)) %>% 
  mutate(Country = "GER")
...
for(country in c("GER", "ENW")){
  for(le in 1:3){
    ...
    par(las = 0)
    if(le == 1 & country == "GER"){
      mtext("West Germany", 3, line = 0.5, cex = 1.4, adj = 0)
    }
    
    if(le == 1 & country == "ENW"){
      mtext("England & Wales without London region", 3, line = 0.5, cex = 1.4, adj = 0)
    }
    ...
    for(age in paste(seq(60, 90, by = 5))){
      ...
      lines(x=plotDat$med_RI+1, y=yy[1]+0.5,
