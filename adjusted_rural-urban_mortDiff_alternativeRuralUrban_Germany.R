...
####################################
########### Germany (using rural (1) and urban (2) distinction)
####################################
GER <- 
  datGER %>% 
  group_by(Age, Sex, Geo, distTyp) %>% 
...
bootstrap <- function(iteration){
  ...
  meanMx <- 
    x1 %>% 
    group_by(Age, distTyp, e60Cat) %>% 
    mutate(mean_mx = mean(mxSim)) %>% 
    distinct(distTyp, e60Cat, mean_mx) %>% 
    # filter(PDcat %in% c(1,3)) %>% 
    pivot_wider(id_cols = c(Age, e60Cat), values_from = "mean_mx", names_from = "distTyp") %>% 
    mutate(mx_diff_RU = (`4`/`1`)-1, 
           mx_diff_RSU = (`3`/`1`)-1,
           mx_diff_RSR = (`2`/`1`)-1,)
  return(meanMx)
}
...
boot_est <- function(input){
  ...
  quant_fct <- function(diffs_RU, diffs_RSU, diffs_RSR){
    values_RU <-  quantile(diffs_RU, probs = c(0.025, 0.5, 0.975))
    values_RSU <-  quantile(diffs_RSU, probs = c(0.025, 0.5, 0.975))
    values_RSR <-  quantile(diffs_RSR, probs = c(0.025, 0.5, 0.975))
    out <- data.frame(low_RU = values_RU[1], med_RU = values_RU[2], up_RU = values_RU[3],
                      low_RSU = values_RSU[1], med_RSU = values_RSU[2], up_RSU = values_RSU[3],
                      low_RSR = values_RSR[1], med_RSR = values_RSR[2], up_RSR = values_RSR[3])
    return(out)
  }
  ...
  est <- 
    ...
    bind_rows() %>%  
    group_by(Age, e60Cat) %>% 
    do(quant_fct(diffs_RU=.$mx_diff_RU, diffs_RSU=.$mx_diff_RSU, diffs_RSR=.$mx_diff_RSR))
  return(est)
  
} 

# Perform bootsrap estimation
mort_diff_adj_GER <- list()

for(i in 1:2){
  input <- GER %>% filter(Sex == i)
  clusterExport(cl, "input")
  mort_diff_adj_GER[[i]] <- boot_est(input)
  mort_diff_adj_GER[[i]]$Sex <- i
}

stopCluster(cl) 

# Combine list to one dataset
mort_diff_adj_GER <- mort_diff_adj_GER %>% bind_rows()
...
pdf("totals_adjusted_Germany_alternativeRuralUrban.pdf", family = "Times", width = 24, height
    = 10, pointsize = 17)
...
for(le in 1:3){
  ...
  for(age in paste(seq(60, 90, by = 5))){
    ...
    lines(x=plotDat$med_RSU+1, y=yy[1]+0.5, typ = "p", pch =3, col = "black")
    lines(x=plotDat$med_RSR+1, y=yy[1]+0.5, typ = "p", pch =15, col = "black")
...
legend(x=1, y=6, legend =c("median rural-semiurban"), col =
         "black", bty = "n", cex = 1.3, pch = 3, horiz = TRUE)
legend(x=1, y=3, legend =c("median rural-semirural"), col =
         "black", bty = "n", cex = 1.3, pch = 15, horiz = TRUE)
