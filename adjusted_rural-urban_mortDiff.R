...
# stopCluster(cl)
 ...
 boot_est <- function(input){
   ...
   quant_fct <- function(diffs){
     values <-  quantile(diffs, probs = c(0.025, 0.5, 0.975))
     out <- data.frame(low = values[1], med = values[2], up = values[3])
     return(out)
}
